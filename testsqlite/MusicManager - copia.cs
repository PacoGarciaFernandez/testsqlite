...
namespace biodanza
{
    public partial class MusicManager : Form
    {
        ...
        private void ConfiguregridCarpetaLista()
        {

            // Grid de proyectos
            if (!configuredgrids[1])
            {
                gridCarpetaLista.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill;
                gridCarpetaLista.CellBorderStyle = DataGridViewCellBorderStyle.None;
                gridCarpetaLista.RowHeadersVisible = false;

                gridCarpetaLista.Columns[0].Visible = false;
                gridCarpetaLista.Columns[1].HeaderText = "Nombre Proyecto";
                gridCarpetaLista.Columns[1].HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleLeft;
        ...
        private void ConfigureGridClaseDef()
        {
            gridClaseDef.RowHeadersVisible = false;
            gridClaseDef.CellBorderStyle = DataGridViewCellBorderStyle.None;
            
            gridClaseDef.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill;
            gridClaseDef.CellBorderStyle = DataGridViewCellBorderStyle.None;
            gridClaseDef.RowHeadersVisible = false;
            
            gridClaseDef.Columns[0].Visible = false;
            gridClaseDef.Columns[1].Visible = true;
            gridClaseDef.Columns[2].Visible = true;
            gridClaseDef.Columns[3].Visible = true;
            gridClaseDef.RowsDefaultCellStyle.BackColor = Color.White;
            gridClaseDef.AlternatingRowsDefaultCellStyle.BackColor = Color.FromArgb(245, 245, 245);
        ...
        void gridItems_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            ...
            if (e.RowIndex > -1 && gridItems.Columns[e.ColumnIndex].Name == "loop")
            {
                // Your code would go here - below is just the code I used to test 
        ...
        private void LoadTree()
        {
            ...
            nodeProyect.Tag = new Tuple<string, Int32>("proyectos", 1);
            nodeProyect.ContextMenu = AddContextMenuProyectos(true);

            LoadgridCarpetaLista();
            ...
            foreach (CarpetaLista cl in lps)
            {
                ...
                // Si id_carpetalista != 0, busco su padre y me añado en el.
                TreeNode padre = nodeProyect;

                if (cl.id_carpetalista != 0)
                {
                    padre = GetNodeById(nodeProyect.Nodes, cl.id_carpetalista);
                }
                //if (padre == null)
                 //   continue;
                ...
                tn.Tag = new Tuple<string, Int32>((cl.tipo==0?"carpeta":"lista"), cl.id);
                tn.ContextMenu = AddContextMenuProyectos(cl.tipo==0);
            }
            nodeProyect.Expand();
        }
        ...
        private void LoadgridCarpetaLista()
        {
            CarpetaListaMng pm = new CarpetaListaMng();
            pm.Attach(gridCarpetaLista);
            ConfiguregridCarpetaLista();
            this.ShowGrid(gridCarpetaLista);
        }
        ...
        {
            //curGrid.Focus();
            //curGrid.Rows[curGrid.CurrentRow.Index + 1].Selected = true;
            //curGrid.Rows[curGrid.SelectedRows[0].Index + 1].Selected = true;
        ...
        private void gridMusica_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            if (e.X <= grid.Columns["loop"].Width && e.Y > grid.ColumnHeadersHeight)
            {
        ...
        private void gridItems_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            if (e.X <= grid.Columns["loop"].Width && e.Y > grid.ColumnHeadersHeight)
            {
                ...
                li.id = Convert.ToInt32(grid.Rows[i].Cells["id"].Value);
                li.id_carpetalista = Convert.ToInt32(grid.Rows[i].Cells["id_carpetalista"].Value);
                li.id_item = Convert.ToInt32(grid.Rows[i].Cells["id_item"].Value);
                li.orden = Convert.ToInt32(grid.Rows[i].Cells["orden"].Value);
                li.repetir = Convert.ToInt32(grid.Rows[i].Cells["repetir"].Value);
                ListaItemsMng lim = new ListaItemsMng();
                lim.Update(li);
            }
            else
        ...
        private void MngProyectoContext(object sender, EventArgs e)
        {
            ...
            else if (text == "Eliminar")
            {
                ...
                {
                    if (MessageBox.Show("Confirmación", "Atención", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
            ...
            //else if (text == "Añadir clase existente...")
            //{
            //    ListaDeClases ldc = new ListaDeClases();
            //    if (ldc.ShowDialog() == DialogResult.OK)
            //    {
            //        ListaClasesProMng lcpm = new ListaClasesProMng();
            //        lista_clases l_c = new lista_clases();
            //        l_c.id_clase = ldc.idClase;
            //        l_c.id_proyecto = ((Tuple<string, Int32>)treeView2.SelectedNode.Tag).Item2;
            //        lcpm.Nuevo(l_c);
            //        LoadTree();
            //        ClaseMng cm = new ClaseMng();

            //        TreeNode node = GetNodeByName(treeView2.Nodes, cm.ClassName(ldc.idClase));
            //        if (node != null)
            //        {
        ...
        private void MngGridMusicaContext(object sender, EventArgs e)
        {
            ...
            {
                Int32 j = gridMusica.SelectedCells[0].RowIndex;
                string file = Path.Combine(gridMusica.Rows[j].Cells["localizacion"].Value.ToString(), gridMusica.Rows[j].Cells["Titulo"].Value.ToString());
        ...
        private void gridClases_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            Int32 j = gridClases.SelectedCells[0].RowIndex;
            Int32 id = Convert.ToInt32(gridClases.Rows[j].Cells["id"].Value);
            TreeNode node = GetNodeById(tvListas.Nodes, id);
            if (node != null)
            {
                tvListas.SelectedNode = node;
                tvListas.Focus();
            }
        }
        ...
        private void añadirCanciónToolStripMenuItem_Click(object sender, EventArgs e)
        {
